...
intrain<-createDataPartition(y=df$Species, p=0.7, list=FALSE) ##train/test data추출 시 유용(각 변수마다 70%씩 추출)
train<-df[intrain, ]
test<-df[-intrain, ]
...
library(rpart.plot)
library(rattle)
bank.rpart <- rpart(y ~ ., data = valid)
fancyRpartPlot(bank.rpart)
# 루트노드가 cci를 기준으로 나뉘는 것으로  보아 cci가 가장 중요한 변수라고 할 수 있다.
# 그 다음은 cpi가 중요한 변수라고 할 수 있다.


predictions <- predict(bank.rpart, valid, type = "class")
confusion.matrix <- prop.table(table(predictions, bank$y))
...
#병렬진행의 좋은 R version: MSR-대용량 자료를 분석할 때 빠르다, 참고
#MSR에 가서 R-server란 버젼을 다운받으면 RevoTreeView패키지 다운 가능
